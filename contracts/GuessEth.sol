...
contract GuessEthEvents {
    ...
    event guessEvt(address indexed playerAddr, uint[] numbers, uint amount);
    event winnersEvt(uint blockNumber, address indexed playerAddr, uint amount, uint winAmount);
    event withdrawEvt(address indexed to, uint256 value);
    event drawEvt(uint indexed blocknumberr, uint number);
    event sponseEvt(address indexed addr, uint amount);
    event pauseGameEvt(bool pause);
    event setOddsEvt(uint odds);
    event setReservefundEvt(uint reservefund);
}
contract GuessEth is Ownable, GuessEthEvents {
    ...
    function setReservefund(uint _reservefund) isHuman() public onlyOwner returns(bool) {
        ...
        emit setReservefundEvt(reservefund);
        return true;
    }
